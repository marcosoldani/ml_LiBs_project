...
$(\text{Frequency [Hz]},\newline \text{Re(Z) [\(\Omega\)]}, \text{Im(Z) [\(\Omega\)]})$.
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \texttt{Z\_imag}      & float & $-\mathrm{Im}(Z)$ in \milliohm{} (convenzione \acrshort{eis}) \\
    \bottomrule
  \end{tabular}
\end{table}
...
volta per processo. Il loader ordina sempre per (Aging, \newline Temperature, 
